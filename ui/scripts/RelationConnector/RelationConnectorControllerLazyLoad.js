var relationConnectorController = function(){
	//This relationConnectorController was developed based on the AframeRelationConnectorController
	...
	async function onRelationsChanged(applicationEvent) {
		...
		let relatedEntitiesSourceEntity = getRelatedEntities(sourceEntity);	
		//unkomment for testing of loading part if Model has already loaded all related entities
		//relatedEntitiesSourceEntity = testFunctionRemoveHalfArray(relatedEntitiesSourceEntity); 

		let relatedEntitiesNeo4j = await neo4jModelLoadController.getRelatedEntities(sourceEntity);
		let relatedEntitiesToLoad = findAllUnloadedRelatedEntities(relatedEntitiesSourceEntity, relatedEntitiesNeo4j);
		
		if(relatedEntitiesToLoad.length > 0){
			events.log.info.publish({ text: "connector - onRelationsChanged - unloaded related Entities - " + relatedEntitiesToLoad.length});

			//because the sourceEntity isn't loaded correctly, we need to load it aswell
			let entitiesToLoad = relatedEntitiesToLoad.concat(sourceEntity);
			model.removeEntity(sourceEntity);
			
			await neo4jModelLoadController.loadEntities(entitiesToLoad);
		}
		
		//now the source Entity should be updated in the model
		sourceEntity = model.getEntityById(sourceEntity.id);
		relatedEntities = getRelatedEntities(sourceEntity);
		...
		if(activated){
			createRelatedConnections(relatedEntities);
			createRelatedEnitities(relatedEntities);
		}
		
	}

	function findAllUnloadedRelatedEntities(relatedEntitiesSourceEntity, relatedEntitiesNeo4j){
		let relatedEntitiesToLoad = new Array;

		for (relatedEntityNeo4j of relatedEntitiesNeo4j){

			let searchId = relatedEntityNeo4j.id;
			
			if (relatedEntitiesSourceEntity.some(relatedEntitySourceEntity => relatedEntitySourceEntity.id == searchId)){
				
				let relatedEntitySourceEntity = relatedEntitiesSourceEntity.find(relatedEntitySourceEntity => {
					return relatedEntitySourceEntity.id === searchId;
				})
				
				if(!relatedEntitySourceEntity.loaded) {
					relatedEntitiesToLoad.push(relatedEntitySourceEntity);
				}
			}
			else{
				relatedEntitiesToLoad.push(relatedEntityNeo4j);
			}
		};

		return relatedEntitiesToLoad;
	}
	


	function createRelatedConnections(relatedEntities){
		...
		relatedEntities.forEach(function(relatedEntity){
			...
			//create model entity
			let elementToJson = {"id":				sourceEntity.id + "--2--" + relatedEntity.id,
								"qualifiedName":	"com.android",
								"name":				sourceEntity.name + " - " + relatedEntity.name,
								"type":				"Relation"
							};
			let element = JSON.parse(JSON.stringify(elementToJson));
			var relation = model.createEntity(element);
	...
	function createRelatedEnitities(relatedEntities){
		// Function similar to PackageExplorer.zTreeOnCheck(), probably Refactoring possible
		let entities = [];
        
		relatedEntities.forEach(function (relatedEntity) {
		    let entity = model.getEntityById(relatedEntity.id)
	...
	function getRelatedEntities(sourceEntity) {
		let relatedEntities = [];
	...
	// this function is used to test the function onRelationsChanged
	// tests the loading part when model has already loaded all related entities by deleting half of the loaded entities form the model
	// function testFunctionRemoveHalfArray(relatedEntitiesToTest) {
	// 	if(relatedEntitiesToTest.length > 1) {
	// 		let cutInHalf = Math.floor(relatedEntitiesToTest.length / 2);
	// 		relatedEntitiesToDelete = relatedEntitiesToTest.splice(0, cutInHalf);
	
	// 		for (relatedEntityToDelete of relatedEntitiesToDelete){
	// 			model.removeEntity(relatedEntityToDelete);
	// 		}
	// 	}
		
	// 	return relatedEntitiesToTest;
	// }
